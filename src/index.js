...
import { ajvError, ValidationError } from '@rugo-vn/exception';
...
const cleanKeyword = function(keyword, value) {
  for (let transform of FINAL_TRANSFORMS) {
...
const walk = function(schema, fn = EmptyFn, traces = []) {
  ...
  for (let keyword in schema) {
    let value = schema[keyword];
    let nextSchemaPart = fn(keyword, value, traces);
    ...
    if (keyword === 'properties') {
      ...
      for (let prop in value) {
        let nextPropSchema = walk(value[prop], fn, [...traces, keyword, prop]);
